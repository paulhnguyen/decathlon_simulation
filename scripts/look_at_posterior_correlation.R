# what are the actual correlations between decathlon events? how does it compare to the posterior predictive distributions generated by the compositional and simple models? uses simulations from code_to_generate_sims.R
# date: aug 28, 2025
set.seed(2)
library(tidyverse)
library(readxl)
...
# find the empirical correlations between each event.
...
# calculate correlations from posterior predictives. have to have run code_to_generate_sims.R to produce the simulations for all athletes.
...
# make seperate dataframe for each iteration before calculating correlation. we have 2000 simulated datasets. we will calculate the correlation between each event for each dataset and compare it to the empirical correlation. we expect that the compositional model reproduces the correlations much better than the simple model.
...
# plot correlations for selected events
